...
export default class Commission extends Component {
    constructor(props) {
        super(props)
        this.state = {
            commissionType: 'dataEntrustMarket',
    ...
    componentDidMount() {
        ...
        this.props.commission(this.setInterval);
        this.setInterval();
    ...
    setInterval(){
        ...
        this[commissionType]({});
        if (this.pollingFetch){
            clearInterval(this.pollingFetch)
        }
        this.pollingFetch = setInterval(() => {
            const {commissionType}=this.state;
            this[JSON.parse(commissionType)]({})
        }, 5000)
    }
    dataEntrustMarketHistory(params) {
        this.props.dispatch({
            type: 'transaction/fetchEntrustMarketHistory',
            payload: params,
            callBack:(data)=>{
             if (data.success){
                 this.setState({dataEntrustMarketHistory:data.data.list})
             }
            }
        })
    }
    dataEntrustMarket(params) {
        this.props.dispatch({
            type: 'transaction/fetchEntrustMarket',
            payload: params,
            callBack:(data)=>{
                if (data.success){
                    this.setState({dataEntrustMarket:data.data.list})
                }
            }
        })
    }

    dataEntrustTracsactionReco(params) {
        this.props.dispatch({
            type: 'transaction/fetchEntrustTracsactionReco',
            payload: params,
            callBack:(data)=>{
                if (data.success){
                    this.setState({dataEntrustTracsactionReco:data.data.list})
                }
            }
        })
    }
    ...
    bannerBodyList(type) {
        ...
        let  mapData=this.state[type];
        if (this.state.hiddenOtherTradeStatus){
            mapData =mapData.filter((coin) => (coin.tradepair === (filters?filters:true)))
        }
        if (type === 'dataEntrustMarket') {
        ...
        } else if (type === 'dataEntrustMarketHistory') {
         return (mapData.map((item, index) => {
                return(
        ...
        } else if (type === 'dataEntrustTracsactionReco') {
           return( mapData.map((item, index) => {
               return(
        ...
        return (
            ...
            >
                <div className={styles.bannerHeader}>
                    <span
                        className={this.state.commissionType === 'dataEntrustMarket' ? styles.actived : ''}
                        onClick={() => this.setState({ commissionType: 'dataEntrustMarket' })
                        }
                    >{intl.get("CURRENT_COMMOSSION")}</span>
                    <span
                        className={this.state.commissionType === 'dataEntrustMarketHistory' ? styles.actived : ''}
                        onClick={() => this.setState({ commissionType: 'dataEntrustMarketHistory' })
                        }
                    >{intl.get("HISTIRIAL_COMMISSION_RECORD")}</span>
                    <span
                        className={this.state.commissionType === 'dataEntrustTracsactionReco' ? styles.actived : ''}
                        onClick={() => this.setState({ commissionType: 'dataEntrustTracsactionReco' })
                        }
