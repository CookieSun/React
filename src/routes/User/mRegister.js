...
import { emailVerify, getPwdVerify, IsPC } from '../../utils'
import styles from './mRegister.less'
...
export default class Mregister extends PureComponent {
    static aa = () => { }
    state = {
        step: 1,
        email: '',
    ...
    componentWillMount() {
        let flag = IsPC()
        if (flag) {
            this.props.dispatch(routerRedux.push(`/newlogin/${this.props.match.params.fid}`))
        }
    }
    componentDidMount() {
    ...
    toShare() {
        this.props.dispatch(routerRedux.push(`/posterPage/${this.state.userRef}`))
    }

    render() {
        return (
            <div className={styles.container}>
                <div className={styles.userBg} style={{ display: this.state.registerStatus ? 'none' : 'block' }}>
                            ...
                            <div className={styles.panelBody} >
                                ...
                                <div className={this.state.step == 2 ? styles.registerInfo : ''}>
                                    ...
                                    <div className={styles.password}>
                                        ...
                                        <p
                                            className={this.state.passwordVerify ? '' : styles.error}
                                        >至少8位，其中必须包含大写字母、特殊符号、小写字母和数字</p>
                                    </div>
                ...
                <div className={styles.registerSuccess} style={{ display: this.state.registerStatus ? 'block' : 'none' }}>
                    <button
